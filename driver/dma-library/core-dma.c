...
static int async_receive_impl(const struct device* dev, void (*callback_func)(void*, void*), size_t data_size, void* user_data, uint8_t* write_adr);
static int sync_receive_impl(const struct device* dev, void* data, size_t data_size, k_timeout_t timeout, uint8_t* write_adr);
...
static int init_core_dma_engine(const struct device* dev) {
	...
	dma_data->smem_data_adr = cfg->smem_base_adr + sizeof(atomic_t) + 4;
  dma_data->smem_control_adr = cfg->smem_base_adr + sizeof(atomic_t);
	if (cfg->m_core) {
    ...
    for (;;) {
      if ((code = send_impl(dev, &master_ready_code, sizeof(master_ready_code), dma_data->smem_control_adr))) {
        return code;
      }
      if (!(code = sync_receive_impl(dev, &receive_byte, sizeof(receive_byte), K_MSEC(50), dma_data->smem_control_adr))) {
		...
		dma_data->core_lock = (atomic_t*)cfg->smem_base_adr;
		uint8_t receive_byte;
		sync_receive_impl(dev, &receive_byte, sizeof(receive_byte), K_FOREVER, dma_data->smem_control_adr);
		if (receive_byte == master_ready_code) {
			if ((code = send_impl(dev, &slave_ready_code, sizeof(slave_ready_code), dma_data->smem_control_adr))) {
				return code;
			}
		} else {
			return -ENODATA;
...
static void async_receive_callback(const struct device *dev, mbox_channel_id_t channel_id,
	...
	memcpy(ret_data, c_data->write_adr, c_data->data_size);
...
static int async_receive_impl(const struct device* dev, void (*callback_func)(void*, void*), size_t data_size, void* user_data, uint8_t* write_adr) {
  ...
  wrap_user_data->write_adr = write_adr;
...
static int sync_receive_impl(const struct device* dev, void* data, size_t data_size, k_timeout_t timeout, uint8_t* write_adr) {
  ...
  memcpy(data, write_adr, data_size);
...
static int sync_receive_impl_wrapper(const struct device* dev, void* data, size_t data_size, k_timeout_t timeout) {
  struct dma_engine_data* dma_data = (struct dma_engine_data*)dev->data;
  return sync_receive_impl(dev, data, data_size, timeout, dma_data->smem_data_adr);
}
...
static int send_impl(const struct device* dev, void* data, size_t data_size, uint8_t write_adr) {
	...
	memcpy(write_adr, data, data_size);
